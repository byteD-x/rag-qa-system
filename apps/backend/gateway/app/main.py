...
AI_PRICE_CURRENCY = (os.getenv("AI_PRICE_CURRENCY", "CNY").strip().upper() or "CNY")
AI_INPUT_PRICE_PER_1K_TOKENS = float(os.getenv("AI_INPUT_PRICE_PER_1K_TOKENS", "0") or 0)
AI_OUTPUT_PRICE_PER_1K_TOKENS = float(os.getenv("AI_OUTPUT_PRICE_PER_1K_TOKENS", "0") or 0)
...
def _load_price_tiers() -> list[PriceTier]:
    raw = os.getenv("AI_PRICE_TIERS_JSON", "").strip()
    if not raw:
        return []
    ...
    try:
        ...
        logger.warning("AI_PRICE_TIERS_JSON is invalid JSON; falling back to flat pricing")
        return []

    if not isinstance(payload, list):
        logger.warning("AI_PRICE_TIERS_JSON must be a JSON array; falling back to flat pricing")
        return []

    tiers: list[PriceTier] = []
    for item in payload:
        if not isinstance(item, dict):
            logger.warning("AI_PRICE_TIERS_JSON contains a non-object entry; skipping it")
            continue
        ...
        if max_input_tokens_raw in (None, ""):
            ...
            try:
                ...
                logger.warning("AI_PRICE_TIERS_JSON has an invalid max_input_tokens value; skipping tier")
                continue
            if max_input_tokens <= 0:
                logger.warning("AI_PRICE_TIERS_JSON max_input_tokens must be positive; skipping tier")
                continue
        ...
        try:
            ...
            logger.warning("AI_PRICE_TIERS_JSON has invalid price values; skipping tier")
            continue

        if input_price < 0 or output_price < 0:
            logger.warning("AI_PRICE_TIERS_JSON price values must be non-negative; skipping tier")
            continue
...
AI_PRICE_TIERS = _load_price_tiers()
...
app = FastAPI(title="RAG-QA 2.0 Gateway", version="3.0.0")
...
def _parse_corpus_id(corpus_id: str) -> tuple[str, str]:
    ...
    raw_id = raw_id.strip()
    if corpus_type not in {"kb", "novel"} or not raw_id:
        raise HTTPException(status_code=400, detail=f"invalid corpus id: {corpus_id}")
    ...
    *,
    ...
    if corpus_type == "kb":
        ...
        payload = await _request_service_json(
            client,
            "GET",
            f"{NOVEL_SERVICE_URL}/api/v1/novel/libraries/{raw_id}/documents",
            headers=headers,
        )
...
async def _fetch_corpora(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        headers = _downstream_headers(user)
        kb_task = _request_service_json(client, "GET", f"{KB_SERVICE_URL}/api/v1/kb/bases", headers=headers)
        novel_task = _request_service_json(client, "GET", f"{NOVEL_SERVICE_URL}/api/v1/novel/libraries", headers=headers)
        kb_payload, novel_payload = await asyncio.gather(kb_task, novel_task)
        ...
        for item in novel_payload.get("items", []) if isinstance(novel_payload, dict) else []:
            corpora.append(
                {
                    "corpus_id": f"novel:{item['id']}",
                    "id": str(item["id"]),
                    "corpus_type": "novel",
                    "name": str(item.get("name") or ""),
                    "description": str(item.get("description") or ""),
            ...
            if corpus_type == "kb":
                ...
                tasks.append(
                    _request_service_json(
                        client,
                        "POST",
                        f"{NOVEL_SERVICE_URL}/api/v1/novel/retrieve",
                        headers=headers,
                        json_body={
                            "library_id": raw_id,
                            "question": contextualized_question,
...
def _estimate_usage_cost(usage: dict[str, Any]) -> dict[str, Any]:
    ...
    if selected_tier is not None:
        ...
        input_price = AI_INPUT_PRICE_PER_1K_TOKENS
        output_price = AI_OUTPUT_PRICE_PER_1K_TOKENS
        pricing_mode = "flat"
...
async def _generate_grounded_answer(
    *,
    ...
    try:
        ...
        logger.warning("ai grounded answer fallback engaged")
        return {
            "answer": _fallback_answer(question, evidence, answer_mode),
            "provider": "",
            "model": "",
