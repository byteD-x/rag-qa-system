...
from .kb_resource_store import list_visual_asset_regions, load_visual_asset
...
@router.get("/api/v1/kb/visual-assets/{asset_id}/regions")
def get_visual_asset_regions(asset_id: str, request: Request, user: CurrentUser) -> dict[str, object]:
    require_kb_permission(request, user, KB_READ_PERMISSION, action="kb.visual_asset.regions", resource_type="visual_asset", resource_id=asset_id)
    return {"items": list_visual_asset_regions(asset_id, user=user)}
