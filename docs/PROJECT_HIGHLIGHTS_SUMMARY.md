...
| 知识库批量写入 | `POST /api/knowledge_base/batch-ingest` 只写入请求体内联文档并索引 section/chunk，不读取本机路径、不上传文件、不批量 rebuild/delete。 | `apps/services/knowledge-base/src/app/kb_batch_ingest.py`、`tests/test_backend_infra.py::test_knowledge_batch_ingest_route_writes_inline_documents_without_raw_content` |
...
- 不把治理页受控 rebuild、batch dry-run 预览或 batch-ingest API 说成文件上传、目录扫描、任意路径重建、批量 rebuild/delete、桌面设置页或后台队列能力。
