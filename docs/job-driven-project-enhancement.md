...
| P1 | 梳理 step events 与 trace 展示 | Agent 可观测 | Gateway 工作流、前端 Agent Monitor 文档/轻量展示 | 把工具调用、失败原因、恢复动作串成可读事件 | Agent 岗位高频追问 | 先做展示和测试，不改深层编排 | 现有 tests + 截图/接口响应 | 能讲多步骤可观测 | 1-2 天 | 前端改动需视觉回归 |
...
- 如果只能投入 1 天：完成本报告、证据校验脚本和演示路径整理。
