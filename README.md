...
聊天页的执行轨迹抽屉会把工作流事件、工具调用、失败原因和恢复目标整理成时间线，便于在面试里直接展示 Agent 可观测性，而不是只翻 JSON 原文。
...
- **工具注册中心**：装饰器注册 + OpenAI/LangChain schema 生成 + 结果缓存 + 执行统计，支持只读维护 dry-run 工具、最终回答摘要工具和本机只读 MCP adapter，并对路径/目标引用做脱敏摘要
...
默认串行执行以保持稳定的 fail-fast 语义；本地需要缩短整套回归时间时，可显式加 `--max-workers 2` 或更高并发度，让多个测试文件分批并行执行。如果怀疑测试进程卡住但没有产生日志，可加 `--idle-timeout-seconds 180 --tail-lines-on-failure 40` 快速失败并打印最近日志。`scripts/quality/ci-check.ps1` 同步支持 `-PytestHeartbeatSeconds`、`-PytestMaxWorkers`、`-PytestIdleTimeoutSeconds`、`-PytestTailLinesOnFailure`、`-PytestArg`、`-PytestSummaryOutput`、`-PytestTargets` 和 `-SkipDoctor`。
PowerShell 质量脚本会通过 `scripts/dev/common.ps1` 优先使用仓库 `.venv` Python，再退回系统 `python` 或 `py -3`，避免全局 Python 缺少 pytest 时误判为项目失败；默认会先运行环境诊断 `scripts/quality/doctor.py`，本机只想跑后续检查时可传 `-SkipDoctor`。
...
```powershell
$targets = python scripts/quality/select_fast_tests.py --staged --fallback tests
powershell -File scripts/quality/ci-check.ps1 -SkipFrontendBuild -SkipFrontendUnitTests -SkipDockerConfig -PytestHeartbeatSeconds 20 -PytestArg "--durations=5" -PytestSummaryOutput logs/quality/ci-check-summary.json -PytestTargets $targets
```

通过 Makefile 运行启动前检查时，也可以用 `PREFLIGHT_ARGS` 透传同一组 pytest runner 参数，例如：

```powershell
make preflight PREFLIGHT_ARGS="-PytestHeartbeatSeconds 5 -PytestTargets tests/test_eval_pipeline.py"
```
